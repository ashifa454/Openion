...
<html lang="en">
  <head>
    ...
    <title><?php echo $title; ?> | Browse All Patetion</title>
        ...
        <div class="android-wear-section">
            ...
            <div class="android-wear-band-text mdl-shadow--2dp" style="display:inline-block;">
              ...
              <div class="mdl-cell mdl-cell--8-col" style=" lear:right; float:left;">
                    <?php
                    if(file_exists('assets/Raw/'.$id.'.jpg')){
                    echo '<img class="article-image" src="../assets/Raw/'.$id.'.jpg" border="0" alt="">';
                    }
                    else{
                      $filename = "assets/Raw/".$id."-*";
                        $rs=glob($filename);
                        $rs=split("-",$rs[0]);
                        $rs=substr($rs[1],0,count($rs[1])-5);
                        echo '<div class="video-container"><iframe src="https://www.youtube.com/embed/'.$rs.'" frameborder="0" allowfullscreen></iframe></div>';
                      }
                    ?>
                <?php
                $file=fopen("control/start-Patetion/".$id."_patetion.txt","r");
                echo fread($file,filesize("control/start-Patetion/".$id."_patetion.txt"));
                ?>
                <hr/>
...
foreach($supportCount as $sp){
  if(isset($_SESSION['user'])&&$sp['u_id']==$_SESSION['user']){
    $flag=1;
  }else if($sp['reason']!=NULL){
...
?>
              ...
              <div class="mdl-cell mdl-cell--4-col" style="float:right;";>
                ...
                <form action="../control/support" method="post">
                  <div class="mdl-textfield mdl-js-textfield reason_box">
                ...
                echo '<Button class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-color--blue-400 mdl-cell--11-col" type="submit">Sign this Petetion
                </Button>';
                }
                else if($flag==1){
                  echo '<a class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-color--blue-400 mdl-cell--11-col" href="login">Share this Petetion
                 </a>';
                }
                else{
                  echo '<a class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-color--blue-400 mdl-cell--11-col" href="login">Login to Sign this Petetion
                </a>';
              }
                ?>
                </form>
  ...
  url(../assets/Raw/'.$pc['id'].'.jpg) bottom right 15% no-repeat; background-size:cover;cursor:pointer;" onclick="showPet('.$pc['id'].')">
  </div>
  <div class="mdl-card__supporting-text">
...
<!--<a href="showPet?petId='.$pc['id'].'" style="text-decoration:none;"><h2 class="mdl-card__title-text">'.$pc['Patetion_title'].'</h2></a>!-->
